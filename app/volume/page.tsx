...
const CHIPS = [
  "Top 10 knee surgeons in PA",
  "Who does the most hips at Jefferson?",
  "ASC vs Hospital volume for knees",
  "Top shoulder surgeons in NJ",
];
...
export default function VolumePage() {
  ...
  async function runQuery(q?: string) {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/volume-query", {
        method: "POST",
  ...
  function downloadCSV() {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "volume-data.csv"; a.click();
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "var(--font-outfit), 'Outfit', sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(150deg, #4a148c 0%, #7b1fa2 60%, #9c27b0 100%)", padding: "20px 16px 18px", color: "#fff" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: -0.5 }}>Volume Intelligence</div>
              <div style={{ fontSize: 12, opacity: 0.7, marginTop: 1 }}>Ask questions about surgical case volume — powered by AI</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "16px 16px 40px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
          {CHIPS.map(chip => (
            <button key={chip} onClick={() => { setQuery(chip); setTimeout(() => runQuery(chip), 50); }}
              style={{ fontSize: 12, padding: "6px 12px", borderRadius: 20, border: `1px solid ${C.border}`, background: "#fff", color: PURPLE, cursor: "pointer", fontFamily: "inherit", fontWeight: 500 }}>
              {chip}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={e => { e.preventDefault(); runQuery(); }} style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          <input
            ...
            placeholder='Ask anything, e.g. "Top knee surgeons in FL"'
          ...
          />
          <button type="submit" disabled={loading} style={{ padding: "0 24px", background: loading ? "#94a3b8" : PURPLE, color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: loading ? "wait" : "pointer", whiteSpace: "nowrap", fontFamily: "inherit" }}>
            {loading ? "…" : "Ask"}
          </button>
        </form>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "24px 0", fontSize: 13, color: C.textSec }}>
            <div style={{ display: "inline-block", width: 20, height: 20, border: "2.5px solid #d1d5db", borderTopColor: PURPLE, borderRadius: "50%", marginRight: 10, verticalAlign: "middle", animation: "spin 0.7s linear infinite" }} />
            Analyzing surgical volume data…
          </div>
        )}
        ...
        {result?.error && (
          <div style={{ padding: "14px 16px", background: result.error.includes("Supabase") ? "#fffde7" : "#fef2f2", borderRadius: 12, color: result.error.includes("Supabase") ? "#5d4037" : "#dc2626", fontSize: 14, fontWeight: 500, border: `1px solid ${result.error.includes("Supabase") ? "#fff9c4" : "#fecaca"}` }}>
            {result.error.includes("Supabase")
              ? <><strong>Supabase not configured.</strong> Add your <code>SUPABASE_KEY</code> to <code>.env.local</code> to enable volume data.</>
              : result.error}
          </div>
        )}
        ...
        {result && !result.error && result.results && (
          <>
            {result.interpretation && (
              <div style={{ padding: "12px 16px", background: "#f5f0ff", borderRadius: 10, border: "1px solid #d8b4fe", fontSize: 14, color: C.brand, fontWeight: 500, marginBottom: 16 }}>
                {result.interpretation}
              </div>
            )}
                  ...
                  <tbody>
                    {result.results.map((row, i) => (
                      <tr key={i} style={{ borderBottom: i < result.results!.length - 1 ? `1px solid ${C.border}` : "none" }}>
                        {result.columns!.map(col => {
                          const val = row[col];
                          const isCases = col === "Total Cases" || col === "Cases";
                          return (
                            <td key={col} style={{ padding: "10px 14px", color: isCases ? PURPLE : C.text, fontWeight: isCases ? 700 : 400, fontFamily: isCases ? "monospace" : "inherit" }}>
                              {col === "Doctor" ? (
