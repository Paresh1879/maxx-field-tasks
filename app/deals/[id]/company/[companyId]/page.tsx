...
export default function CompanyDetailPage() {
  const { id: dealId, companyId } = useParams<{ id: string; companyId: string }>();
  ...
  const hubspotUrl = `https://app.hubspot.com/contacts/${companyId}`;
    ...
    <div className="min-h-screen" style={{ background: "#f4f8fb" }}>
      <div style={{ background: "linear-gradient(150deg, #0c2d48 0%, #1565a0 60%, #2e86c1 100%)" }}>
        <div className="max-w-lg mx-auto px-4 py-4">
          ...
          {company?.domain && (
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", marginTop: 4 }}>{company.domain}</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-5 pb-12">
        {error && (
          <div style={{ padding: "12px 16px", background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 12 }}>
            <p style={{ fontSize: 13, color: "#dc2626" }}>Could not load company details.</p>
          </div>
        )}
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #dce4ec", overflow: "hidden" }}>
            <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{
                  width: 44, height: 44, borderRadius: 10, background: "#e8f0f8",
                  display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                }}>
                  <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="#1565a0" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                </div>
                <div>
                  <p style={{ fontSize: 16, fontWeight: 700, color: "#0c2d48" }}>{company.name}</p>
                  {location && <p style={{ fontSize: 13, color: "#64748b" }}>{location}</p>}
                </div>
              </div>
                ...
                <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                  {company.domain && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <a
                        ...
                        style={{ fontSize: 14, color: "#1565a0", textDecoration: "none" }}
                      >
                        {company.domain}
                      </a>
                    </div>
                  ...
                  {company.phone && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <a href={`tel:${company.phone}`} style={{ fontSize: 14, color: "#1565a0", textDecoration: "none" }}>
                        {company.phone}
                      </a>
                    </div>
                  )}
