...
type Provider = {
  ...
  specialty: string; addresses: { city: string; state: string; zip?: string; phone?: string }[];
};
type Payments = {
  total: number; records: number; npi: number | null; specialty: string;
  byYear: { year: string; amount: number }[];
  byCompany: { company: string; amount: number }[];
  byType: { type: string; amount: number }[];
};
type Volume = {
  totalQty: number; doctorName: string;
  bySurgery: { surgery: string; qty: number }[];
  byFacility: { name: string; qty: number; type: string }[];
  bySetting: { Hospital: number; ASC: number };
};
type Bio = {
  summary: string;
  education: { medical_school?: string; residency?: string; fellowship?: string | null };
  hospital_affiliations: string[];
  clinical_focus: string[];
  board_certifications: string[];
  research_interests: string[];
  industry_relationships: string | null;
};
...
/* ── Tooltip formatter ── */
function dollarTip(value: number) { return fmtDollars(value); }
function casesTip(value: number) { return `${value.toLocaleString()} cases`; }
...
/**
 * Strip credentials and extract a clean { first, last } from HubSpot contact names.
 * HubSpot often stores the full credentialed name in both firstname AND lastname
 * (e.g. "Michael J. Taunton, MD" / "Michael J. Taunton, MD"), which breaks
 * Supabase queries that expect just "Taunton" in the last_name column.
 */
function extractCleanName(firstname: string, lastname: string): { first: string; last: string } {
  const CREDS = /,?\s*(M\.?D\.?|D\.?O\.?|M\.?B\.?B\.?S\.?|M\.?P\.?H\.?|Ph\.?D\.?|D\.?P\.?M\.?|M\.?S\.?|F\.?A\.?C\.?S\.?|F\.?A\.?A\.?O\.?S\.?|Jr\.?|Sr\.?|II+|IV)\b/gi;
  const strip = (s: string) => s.replace(CREDS, "").replace(/,+\s*$/, "").trim();

  const fn = strip(firstname);
  const ln = strip(lastname);

  // If both cleaned values are the same (or lastname is empty), parse fn as a full name
  if (!ln || fn.toLowerCase() === ln.toLowerCase()) {
    const parts = fn.split(/\s+/).filter(Boolean);
    // Drop lone middle initials (single letter + optional dot)
    const meaningful = parts.filter(p => !/^[A-Z]\.?$/.test(p));
    if (meaningful.length === 0) return { first: fn, last: "" };
    if (meaningful.length === 1) return { first: meaningful[0], last: "" };
    return { first: meaningful[0], last: meaningful[meaningful.length - 1] };
  }

  // If fn already contains ln (full name stored as firstname), parse fn
  if (fn.toLowerCase().includes(ln.toLowerCase())) {
    const parts = fn.split(/\s+/).filter(Boolean);
    const meaningful = parts.filter(p => !/^[A-Z]\.?$/.test(p));
    return { first: meaningful[0] || fn, last: meaningful[meaningful.length - 1] || "" };
  }
...
/* ── Main component ── */
export default function SurgeonProfileClient({ contact }: { contact: Contact }) {
  const clean = extractCleanName(contact.firstname, contact.lastname);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<ProfileData | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!clean.first && !clean.last) return;
    setLoading(true);
    const params = new URLSearchParams();
    if (clean.first) params.set("first_name", clean.first);
    params.set("last_name", clean.last || clean.first);
  ...
  const displayName = provider
    ...
    : `${clean.first} ${clean.last}`.trim().toUpperCase();
  ...
  return (
    <>
      ...
      {/* ── Single gradient block: back button + title + search form ── */}
      <div style={{ background: "linear-gradient(150deg, #0d47a1 0%, #1565c0 60%, #1e88e5 100%)" }}>
        {/* Header row */}
        <div style={{ maxWidth: 700, margin: "0 auto", padding: "20px 16px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <a href="/surgeons" style={{
              background: "rgba(255,255,255,0.15)", borderRadius: 8,
              color: "#fff", padding: "10px 12px", display: "flex",
              alignItems: "center", textDecoration: "none", minWidth: 44, justifyContent: "center",
            }}>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "16px 16px 32px" }} id="profile-content">
        ...
        {loading && (
          <Section>
            <div style={{ textAlign: "center", padding: "24px 0", fontSize: 13, color: "#6b7280", fontWeight: 500 }}>
              ...
              Building surgeon profile… This may take a few seconds.
            </div>
          </Section>
        )}

        {error && (
          <div style={{ padding: "14px 16px", background: "#fef2f2", borderRadius: 14, color: "#dc2626", fontSize: 14, fontWeight: 500, border: "1px solid #fecaca", marginBottom: 14 }}>
            {error}
          </div>
        )}
        ...
        {data && !loading && (() => {
          ...
          const location = addr ? `${addr.city}, ${addr.state} ${addr.zip?.slice(0,5) || ""}`.trim() : "";
            ...
            <>
              ...
              <Section>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                  <div>
                    ...
                    {contact.email && <a href={`mailto:${contact.email}`} style={{ display: "block", fontSize: 13, color: PRIMARY, marginTop: 6, textDecoration: "none" }}>{contact.email}</a>}
                    {contact.phone && <a href={`tel:${contact.phone}`} style={{ display: "block", fontSize: 13, color: PRIMARY, marginTop: 2, textDecoration: "none" }}>{contact.phone}</a>}
                  </div>
                  <button className="no-print" onClick={() => window.print()}
                    style={{ background: PRIMARY, color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
                    Print Report
                  </button>
                </div>
              </Section>
              ...
              {bio && (
                <Section title="Professional Summary" color={PRIMARY}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {bio.education.medical_school && (
                          <div style={{ display: "flex", gap: 10 }}>
                            <Dot color={PRIMARY} />
                            <div><div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase" }}>Medical School</div><div style={{ fontSize: 14, color: BRAND, fontWeight: 500 }}>{bio.education.medical_school}</div></div>
                          </div>
                        )}
                        {bio.education.residency && (
                          <div style={{ display: "flex", gap: 10 }}>
                            <Dot color={PRIMARY} />
                            <div><div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase" }}>Residency</div><div style={{ fontSize: 14, color: BRAND, fontWeight: 500 }}>{bio.education.residency}</div></div>
                          </div>
                        )}
                        {bio.education.fellowship && (
                          <div style={{ display: "flex", gap: 10 }}>
                            <Dot color={PRIMARY} />
                            <div><div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase" }}>Fellowship</div><div style={{ fontSize: 14, color: BRAND, fontWeight: 500 }}>{bio.education.fellowship}</div></div>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Clinical Focus + Board Certs grid */}
                      ...
                      {bio.clinical_focus?.length > 0 && (
                        <div>
                          ...
                          {bio.clinical_focus.map((f, i) => (
                            <div key={i} style={{ fontSize: 13, color: BRAND, marginBottom: 4, display: "flex", alignItems: "flex-start", gap: 6 }}>
                      ...
                      {bio.board_certifications?.length > 0 && (
                        <div>
                          ...
                          {bio.board_certifications.map((c, i) => (
                            <div key={i} style={{ fontSize: 13, color: BRAND, marginBottom: 4, display: "flex", alignItems: "flex-start", gap: 6 }}>
                              <span style={{ color: GREEN, fontWeight: 700 }}>—</span> {c}
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  {bio.hospital_affiliations?.length > 0 && (
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {bio.hospital_affiliations.map((h, i) => (
                          <span key={i} style={{ fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 8, background: "#dce8f5", color: PRIMARY }}>{h}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {bio.research_interests?.length > 0 && (
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {bio.research_interests.map((r, i) => (
                          <span key={i} style={{ fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 8, background: "#f3e5f5", color: PURPLE }}>{r}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {volume && volume.totalQty > 0 && (
                <Section title="Surgical Volume" color={PURPLE}>
                  ...
                  {/* By Surgery Type — horizontal bar chart */}
                  {volume.bySurgery.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <ResponsiveContainer width="100%" height={Math.max(80, volume.bySurgery.slice(0, 6).length * 36)}>
                        <BarChart
                          data={volume.bySurgery.slice(0, 6)}
                          layout="vertical"
                        ...
                        >
                          ...
                          <Tooltip formatter={(v: unknown) => [casesTip(v as number), ""]} cursor={{ fill: "#f3e5f5" }} />
              ...
              {payments && payments.total > 0 && (
                <Section title="Industry Payments" color={GREEN}>
                  ...
                  {/* Payments by Year — vertical bar chart */}
                    ...
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={[...payments.byYear].reverse()} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                          ...
                          <Tooltip formatter={(v: unknown) => [dollarTip(v as number), "Payments"]} cursor={{ fill: "#e8f5e9" }} />
                          <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                            {[...payments.byYear].reverse().map((_, i, arr) => (
                              <Cell key={i} fill={i === arr.length - 1 ? "#1b5e20" : "#43a047"} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                  ...
                  {payments.byCompany.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <ResponsiveContainer width="100%" height={Math.max(80, payments.byCompany.slice(0, 8).length * 36)}>
                        <BarChart
                          data={payments.byCompany.slice(0, 8)}
                          layout="vertical"
                          margin={{ top: 0, right: 80, bottom: 0, left: 5 }}
                        >
        ...
        {/* Privacy policy */}
        <div className="no-print" style={{ marginTop: 24, paddingTop: 16, borderTop: `1px solid ${BORDER}`, fontSize: 11, color: "#94a3b8", lineHeight: 1.7, textAlign: "center" }}>
          <div style={{ fontWeight: 700, marginBottom: 4, color: "#374151" }}>Maxx HubApp</div>
          <div style={{ marginBottom: 4 }}>© 2026 Maxx Orthopedics. All rights reserved.</div>
          <a href="https://maxxortho.com/privacy-policy/" target="_blank" rel="noopener noreferrer" style={{ color: "#1565a0", textDecoration: "none", fontSize: 11, fontWeight: 600 }}>Privacy Policy</a>
          <div style={{ marginTop: 8, fontSize: 11, color: "#94a3b8" }}>
            NPI data sourced from CMS NPPES. AI-generated content may contain inaccuracies — for informational purposes only.
          </div>
        </div>
      </div>
    </>
